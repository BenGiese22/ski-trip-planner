...
3. **Destination preference is one select control**, not a separate toggle — it feeds `destination_votes` (rank 1) and the cost section just displays the chosen destination's name as text, driving which numbers show. Full multi-destination ranking is deferred past Phase 2. *[Revised during Phase 3: the dropdown sat below the cards and was easy to miss, and gave no clue what picking one did. It is now a "Prefer this one" toggle on each destination card, where people are already reading about the option, with copy saying it tailors the cost estimate and binds nobody. The pick can also be taken back, so `destinationSlug` is nullable in the patch schema and clearing it deletes the vote row. Still a single rank-1 choice — ranking remains deferred.]*
